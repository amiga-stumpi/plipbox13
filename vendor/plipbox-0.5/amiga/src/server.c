...
PRIVATE ULONG plipbox_dbg_strlen(const char *s)
{
   ULONG n = 0;
   if (!s) return 0;
   while (s[n]) ++n;
...
PRIVATE VOID plipbox_dbg_puts(BASEPTR, const char *s)
{
   ULONG len;
   BPTR out;
   if (!DOSBase || !s) return;
   out = Output();
   len = plipbox_dbg_strlen(s);
   if (out && len) Write(out, (APTR)s, (LONG)len);
}
#define PLIP_DBG(s) plipbox_dbg_puts(pb, (s))
#else
#define PLIP_DBG(s) ((VOID)0)
#endif
...
/*F*/ PRIVATE REGARGS BOOL goonline(BASEPTR)
{
   ...
   PLIP_DBG("PLIP05 goonline\n");

   if (pb->pb_Flags & PLIPF_OFFLINE)
   {
#ifdef PLIPBOX_OS13_NO_ATTACH
      PLIP_DBG("PLIP05 NO_ATTACH online shortcut\n");
      GetSysTime(&pb->pb_DevStats.LastStart);
      pb->pb_Flags &= ~PLIPF_OFFLINE;
      DoEvent(pb, S2EVENT_ONLINE);
#else
      PLIP_DBG("PLIP05 hw_attach begin\n");
      if (!hw_attach(pb))
      {
      ...
      {
         PLIP_DBG("PLIP05 hw_attach ok\n");
         PLIP_DBG("PLIP05 magic online begin\n");
         if(send_magic_pkt(pb, MAGIC_ONLINE)) {
            PLIP_DBG("PLIP05 magic online ok\n");
            GetSysTime(&pb->pb_DevStats.LastStart);
            pb->pb_Flags &= ~PLIPF_OFFLINE;
            DoEvent(pb, S2EVENT_ONLINE);
            d(("i'm now online!\n"));
         } else {
            PLIP_DBG("PLIP05 magic online failed\n");
            d(("hello pkt failed!\n"));
            rc = FALSE;
         }
      }
...
/*F*/ PRIVATE REGARGS AW_RESULT write_frame(BASEPTR, struct IOSana2Req *ios2)
{
   ...
   if (!plipbox_call_bm(bm->bm_CopyFromBuffer, frame_ptr,
...
#ifdef PLIPBOX_GCC_DIRECT
   if (!plipbox_call_bm(bm->bm_CopyToBuffer, req->ios2_Data, frame_ptr, datasize))
#else
...
/*F*/ PRIVATE REGARGS VOID dos2reqs(BASEPTR)
{
   ...
   */
   ...
   {
#ifdef PLIPBOX_OS13_PACKET_DEBUG
      PLIP_DBG("PLIP05 server got msg\n");
#endif
      if (hw_recv_pending(pb))
      {
         d(("incoming data!"));
      ...
      switch (ios2->ios2_Req.io_Command)
      {
         ...
         case S2_CONFIGINTERFACE:
            PLIP_DBG("PLIP05 S2_CONFIGINTERFACE server\n");
#ifdef PLIPBOX_OS13_CONFIG_NOOP
            PLIP_DBG("PLIP05 CONFIG_NOOP return ok\n");
...
/*F*/ PRIVATE struct PLIPBase *startup(void)
{
   ...
   base = ss->ss_PLIPBase;
#ifdef PLIPBOX_GCC_DIRECT
   plipbox_sync_global_bases(base);
#endif
   base->pb_Startup = ss;
...
#ifdef PLIPBOX_OS13
   /*
   ** Kickstart 1.3 build: keep the original 0.5 defaults and avoid ReadArgs()
   ** ENV: dependencies in the server task.  A small S:plipbox.config parser can
   ** be added later, but the protocol path must remain plipbox-0.5 exact.
   */
   pb->pb_CollisionDelay = PLIP_DEFDELAY + (pb->pb_Unit ? PLIP_DELAYDIFF : 0);
...
/*F*/ PUBLIC VOID SAVEDS ServerTask(void)
{
   ...
   {      
      ...
      if (init(pb))
      {
         ...
         portsigmask  = 1UL << pb->pb_ServerPort->mp_SigBit;
         ...
         for(running=TRUE;running;)
         {
            d4(("** wmask is 0x%08lx\n", wmask));
#ifdef PLIPBOX_OS13_CONFIG_NOOP
            PLIP_DBG("PLIP05 noop wait\n");
            recv = Wait(SIGBREAKF_CTRL_F | SIGBREAKF_CTRL_C | portsigmask);
            PLIP_DBG("PLIP05 noop woke\n");
            ...
            {
               struct Message *msg;
               while ((msg = GetMsg(pb->pb_ServerPort)))
               {
                  if (msg == &pb->pb_ServerWakeMsg)
                  {
                     pb->pb_ServerWakePending = 0;
                     PLIP_DBG("PLIP05 wake msg consumed\n");
                  }
                  else
                  {
                     AddHead(&pb->pb_ServerPort->mp_MsgList, (struct Node *)msg);
                     break;
                  }
               }
               dos2reqs(pb);
            ...
            /* handle SANA-II requests.  On Kickstart 1.3 the MsgPort signal
            ** path is fragile in this GCC port, so also check the server port
            ** on explicit wakeups used by DevForwardIO().
            */
#ifdef PLIPBOX_OS13
            if ((recv & (portsigmask | SIGBREAKF_CTRL_F)) ||
                (pb->pb_ServerPort && pb->pb_ServerPort->mp_MsgList.lh_Head->ln_Succ))
#else
            if (recv & portsigmask)
#endif
            {
