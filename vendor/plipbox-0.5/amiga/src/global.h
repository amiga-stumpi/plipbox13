...
typedef APTR BMFunc;
BOOL plipbox_call_bm(BMFunc func, void *dst, void *src, LONG size);
#else
...
#ifdef PLIPBOX_OS13
   struct Message              pb_ServerWakeMsg;       /* port wake for OS1.3 */
   UBYTE                       pb_ServerWakePending;
   UBYTE                       pb_pad_server_wake[3];
#endif
...
#  ifdef PLIPBOX_GCC_DIRECT
VOID plipbox_sync_global_bases(BASEPTR);
#  endif
#else
...
/****************************************************************************/
   ...
   */
#ifdef PLIPBOX_OS13
#define CONFIGFILE "S:plipbox.config"
#else
